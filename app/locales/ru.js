export default {
  ...
  aboutPages: {
    missingContent: {
      ...
      results:
        'По этому проекту пока нет результатов, о которых можно было бы сообщить.',
  ...
  classifier: {
    back: 'назад',
    backButtonWarning:
      'Если Вы нажмете "назад", Ваша работа по текущему заданию не сохранится',
    close: 'Закрыть',
    continue: 'Продолжить',
    ...
    workflowAssignmentDialog: {
      promotionMessage:
        'Поздравляем! Вы открыли следующий уровень. Если Вы предпочитаете оставаться на этом уровне, можно выбрать "остаться"',
  ...
  project: {
    ...
    disclaimer:
      'Этот проект был создан с использованием Zooniverse Project Builder, но еще не является официальным проектом Zooniverse. Поэтому команда Zooniverse не сможет Вам ответить на вопросы, связанные с этим проектом',
    about: {
      header: 'Узнать о',
      nav: {
    ...
    home: {
      ...
      talk: {
        ...
        other:
          'В данный момент %(count)s участник (а, ков) обсуждает (ют) %(title)s'
      },
  ...
  projects: {
    welcome: {
      ...
      thanks:
        'Спасибо за ваш интерес к помощи реальному исследованию. Здесь мы собрали несколько проектов, в которых мы действительно могли бы использовать вашу помощь прямо сейчас. Для изучения дополнительных возможностей просто прокрутите вниз, чтобы просмотреть все активные проекты.'
    }
  },
  organization: {
    ...
    notPermission:
      ' Вы уверены, что ссылка введена правильно? Возможно, у Вас нет разрешения на доступ к этой организации',
  ...
  signIn: {
    ...
    whyHaveAccount:
      'Авторизованные волонтеры могут получить доступ к своей работе. Их поблагодарят в итоговой публикации',
    signIn: 'Войти',
  ...
  resetPassword: {
    ...
    newPasswordFormDialog:
      'Чтобы продолжить исследование, введите пароль и подтвердите его. В пароле должно быть не менее 8 символов',
    newPasswordFormLabel: 'Новый пароль',
    newPasswordConfirmationLabel: 'Для подтверждения повторите пароль',
    enterEmailLabel:
      'Пожалуйста, введите здесь свой имейл адрес, и мы пошлем ссылку, по которой можно будет изменить пароль',
    emailSuccess:
      'Мы послали Вам ссылку, по которой можно будет изменить пароль',
    emailError: 'Ошибка изменения пароля',
    passwordsDoNotMatch: 'Пароль не совпадает, пожалуйста, повторите',
    loggedInDialog:
      'Вы активизированы в своем аккаунте. Пожалуйста, выйдите, чтобы изменить пароль',
    missingEmailsSpamNote:
      'Если Вы не получили имейл со ссылкой для изменения пароля, пожалуйста, проверьте папку спам',
    missingEmailsAlternateNote:
      'Если Вы все еще не получили имейл, пожалуйста, введите другой имейл, с которым Вы могли войти в свой аккаунт'
  },
  workflowToggle: {
    label: 'Активно'
  },
    ...
    talk: {
      ...
      options: {
        participating_discussions:
          'Когда появятся новости в обсуждении, в котором я участвую',
        followed_discussions:
          'Когда появятся новости в обсуждении, за которым я наблюдаю',
        ...
        started_discussions:
          'Когда обсуждение началось на доске, за которой я наблюдаю'
      }
    },
    project: {
  ...
  userSettings: {
    account: {
      ...
      displayNameHelp:
        'Как Ваше имя будет показано другим участникам Обсуждения [Talk] и на Вашем профиле',
      realName: 'Настоящее имя (указать по желанию)',
      realNameHelp:
        'Публичное пространство; мы используем это пространство, чтобы сделать ссылки на проект в статьях, постерах и других материалах',
      interventions: 'Показывать уведомления об изменениях в проекте.',
      interventionsHelp:
        'Разрешить показывать сообщения, когда Вы работаете над разметкой/классификацией',
    ...
    profile: {
      ...
      profileHeaderImageHelp:
        'Скинуть картинку здесь (любой размер, менее %(size)s KB)',
