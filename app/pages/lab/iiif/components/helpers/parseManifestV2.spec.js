...
describe('parseManifestV2', () => {
  ...
  it('should record the manifest identifier', () => {
    expect(data.metadata['iiif:manifest']).to.equal(manifest['@id']);
  });

  it('should generate subjects from the default sequence', () => {
    expect(data.subjects.length).to.equal(sequence.canvases.length);
  });

  it('should assign a priority to each subject', () => {
  ...
  it('should assign locations to each subject from the canvas images', () => {
    sequence.canvases.forEach((canvas, index) => {
      const subject = data.subjects[index];
  ...
  it('should assign canvas identifiers to each subject', () => {
    sequence.canvases.forEach((canvas, index) => {
      const subject = data.subjects[index];
  ...
  describe('metadata', () => {
    it('should convert manifest metadata to Markdown', () => {
    ...
    it('should parse metadata items in multiple languages', () => {
      const mockManifest = {
        metadata: [
          {
            label: 'Published',
    ...
    it('should parse metadata labels in multiple languages', () => {
      const mockManifest = {
        metadata: [
          {
            label: [
