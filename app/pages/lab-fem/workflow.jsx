...
class EditWorkflowPage extends Component {
  ...
  workflowLink() {
    ...
    if (isThisProjectUsingFEMLab(this.props.project, this.props.location)) {
      env = process.env.NODE_ENV;
      if (env === 'production') {
        return `${FEM_LAB_PREVIEW_HOST}/projects/${owner}/${name}/classify/workflow/${this.props.workflow.id}`;
      } else {
        return `${FEM_LAB_PREVIEW_HOST}/projects/${owner}/${name}/classify/workflow/${this.props.workflow.id}?env=${env}`;
      }

    // WARNING: transcription-task case may no longer be correct as of Dec 2021
    } else if (this.canUseTask(this.props.project, 'transcription-task') && usingTranscriptionTask) {
      env = process.env.NODE_ENV;
  ...
  render() {
    ...
    if (persist_annotations === undefined) { persist_annotations = true; }

    let {
      hide_classification_summaries
    } = this.props.workflow.configuration;
          ...
          ? (
            <p className="form-help warning">
              ...
              You cannot edit tasks for an active workflow on a live project, all the task fields below will be inactive.
                <br />
              To edit these fields, deactivate your workflow on the
                {' '}
                <Link to={`/lab/${this.props.project.id}/workflows`}>workflows page</Link>
                {' '}
- do not change your project to development.
              </strong>
            </p>
          ) : undefined}
              ...
              <div className={disabledIfLive}>
                <div className="nav-list standalone">
                  ...
                  {noTasksDefined
                    ...
                    : (() => {
                      const result = [];
                      for (const key in this.props.workflow.tasks) {
                            ...
                            >
                              {(() => {
                                switch (definition.type) {
                                  case 'single': return <i className="fa fa-dot-circle-o fa-fw" />;
                                  case 'multiple': return <i className="fa fa-check-square-o fa-fw" />;
                                  case 'drawing': return <i className="fa fa-pencil fa-fw" />;
                                  case 'survey': return <i className="fa fa-binoculars fa-fw" />;
                                  case 'flexibleSurvey': return <i className="fa fa-binoculars fa-fw" />;
                                  case 'crop': return <i className="fa fa-crop fa-fw" />;
                                  case 'text': return <i className="fa fa-file-text-o fa-fw" />;
                                  case 'textFromSubject': return <i className="fa fa-file-text-o fa-fw" />;
                                  case 'dropdown': return <i className="fa fa-list fa-fw" />;
                                  case 'combo': return <i className="fa fa-cubes fa-fw" />;
                                  case 'slider': return <i className="fa fa-sliders fa-fw" />;
                                  case 'highlighter': return <i className="fa fa-i-cursor" />;
                                  case 'transcription': return <i className="fa fa-font fa-fw" />;
                                }
                              })()}
                              {' '}
                  ...
                  {this.state.showTaskAddButtons
                    ? (
                      <div className="edit-workflow-page__section edit-workflow-page__task-add-buttons">
                        ...
                        {' '}
                        {this.canUseTask(this.props.project, 'textFromSubject')
                          ? (
                            <AutoSave resource={this.props.workflow}>
                              <button type="button" className="minor-button" onClick={this.addNewTask.bind(this, 'textFromSubject')} title="Text from subject tasks: the volunteer corrects text provided by subject text location.">
                                <i className="fa fa-file-text-o fa-2x" />
                                <br />
                          ...
                          ? (
                            <AutoSave resource={this.props.workflow}>
                              <button type="button" className="minor-button" onClick={this.addNewTask.bind(this, 'highlighter')} title="Highlighter: The volunteer can highlight piece of text.">
                                <i className="fa fa-i-cursor fa-2x" />
                                <br />
...
Version
                  {this.props.workflow.version}
                  {' '}
- Status:
                  <span className={this.props.workflow.active ? 'color-label green' : 'color-label red'}>{this.props.workflow.active ? 'Active' : 'Inactive'}</span>
                </small>
              </p>
            ...
            <div className="deprecated">
              <AutoSave resource={this.props.workflow}>
                <span className="form-label">Set annotation persistence</span>
                <br />
            ...
            <div className="deprecated">
              <AutoSave resource={this.props.project}>
                <span className="form-label">Set as default</span>
                <br />
                ...
                {projectLiveWorkflowInactive ? (
                  <span>
                    <br />
                    <small className="form-help">Inactive workflows on live projects cannot be made default.</small>
                  </span>
                ) : undefined}
                <br />
                <label>
...
Associated tutorial
                {this.props.project?.experimental_tools.includes('mini-course') ? 'and/or mini-course' : undefined}
              </span>
              <br />
              <small className="form-help">
Choose the tutorial
                {this.props.project?.experimental_tools.includes('mini-course') ? 'and/or mini-course' : undefined}
                {' '}
you want to use for this workflow.
              </small>
              <br />
            ...
            <div>
              <div className="deprecated">
                <AutoSave resource={this.props.workflow}>
                  <span className="form-label">Classification summaries</span>
                  <br />
            ...
            {Array.from(this.props.project.experimental_tools).includes('sim notification')
              ? (
                <div className="unsupported">
                  <div>
                    <AutoSave resource={this.props.workflow}>
                      <span className="form-label">Simulation subject notification</span>
                      <br />
                      <small className="form-help">Simulation subject notification will display a small message in the classification summary that lets the volunteer know the subject is a simulation.</small>
                      <br />
                      <br />
                      <small className="form-help">
For this feature to work, it requires hidden subject metadata with the column label
                        <code>#sim</code>
                        {' '}
...
or
            ...
            {Array.from(this.props.project.experimental_tools).includes('Gravity Spy Gold Standard')
              ? (
                <div className="deprecated">
                  <div>
                    <AutoSave resource={this.props.workflow}>
                      <span className="form-label">Gravity Spy Gold Standard</span>
                      <br />
            ...
            <div>
              <div className="deprecated">
                <AutoSave resource={this.props.workflow}>
                  <span className="form-label">Pan and zoom</span>
                  <br />
            ...
            <div className="deprecated">
              <AutoSave tag="div" resource={this.props.workflow}>
                <span className="form-label">Multi-image options</span>
                <br />
            ...
            {Array.from(this.props.project.experimental_tools).includes('enable subject flags')
              ? (
                <div>
                  <div className="unsupported">
                    <AutoSave resource={this.props.workflow}>
                      <span className="form-label">Subject Flags</span>
                      <br />
            ...
            <p>
              ...
              <br />
              <small className="form-help">
If you&apos;d like more complex retirement rules, please get in touch via the
...
page.
            ...
            {Array.from(this.props.project.experimental_tools).includes('worldwide telescope')
              ? (
                <div>
                  <div className="deprecated">
                    <AutoSave resource={this.props.workflow}>
                      <span className="form-label">Use World Wide Telescope API</span>
                      <br />
  ...
  handleSetPanAndZoom(e) {
    return this.props.workflow.update({ 'configuration.pan_and_zoom': e.target.checked });
  }

  handleSetHideClassificationSummaries(e) {
    return this.props.workflow.update({ 'configuration.hide_classification_summaries': e.target.checked });
  }

  handlePersistAnnotationsToggle(e) {
    return this.props.workflow.update({ 'configuration.persist_annotations': e.target.checked });
  }

  handleSetSimNotification(e) {
    return this.props.workflow.update({ 'configuration.sim_notification': e.target.checked });
  }

  handleSetInvert(e) {
    return this.props.workflow.update({ 'configuration.invert_subject': e.target.checked });
  }

  handleSetGravitySpyGoldStandard(e) {
    return this.props.workflow.update({ 'configuration.gravity_spy_gold_standard': e.target.checked });
  }

  enableSubjectFlags(e) {
    return this.props.workflow.update({ 'configuration.enable_subject_flags': e.target.checked });
  }

  handleSetWorldWideTelescope(e) {
    if (!this.props.workflow.configuration.custom_summary) {
      this.props.workflow.update({ 'configuration.custom_summary': [] });
    }
    ...
    return this.props.workflow.update({ 'configuration.custom_summary': summary_path });
  }
  ...
  handleDelete() {
    ...
    if (confirmed) {
      ...
      }).catch(error => this.setState({ deletionError: error })).then(() => {
        if (this.isMounted()) {
  ...
  handleViewClick() {
    return setTimeout(() => this.setState({ forceReloader: this.state.forceReloader + 1 }));
  }
