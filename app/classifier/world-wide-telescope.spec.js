...
describe('WorldWideTelescope render without incomplete annotations', () => {
  it('will render an empty div with no annotations', () => {
    const page = shallow(<WorldWideTelescope subject={testSubject} />);
    assert.equal(page.find('div').children().length, 0);
  });

  it('will render an empty div with incomplete annotations', () => {
...
xdescribe('WorldWideTelescope with classification', () => {
  let wrapper;
  ...
  it('will render a WWT link for each full classification', () => {
    const linkInstances = wrapper.find('.standard-button');
    assert.equal(linkInstances.length, 2);
  });

  it('will render a cropped image for each fully classified chart', () => {
    const image = wrapper.find('img');
    assert.equal(image.length, 2);
  });

  it('will render text about misaligned images', () => {
