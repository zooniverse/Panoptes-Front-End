...
If your PR is open for a while, it may get out of synch with the `main` branch. In that case, pull the latest `main` branch and run `git rebase main` – while in your _feature_ branch – to update your feature branch to the latest `main`. You may need to use `git push --force-with-lease` to update your PR after rebasing.
...
### PFE Pull Request Workflow
  ...
  - When the PR is ready, Person B clicks approve,
  - Person A merges their own PR, unless Person A is an external contributor to the Zooniverse Github org.
